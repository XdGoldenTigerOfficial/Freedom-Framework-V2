...
namespace client.Modules
{
    public class Activities : BaseScript
    {
        ...
        private static async Task CheckMarkers()
        {
            foreach (Vector3 location in MovieTheaterLocations)
            {
                float Distance = World.GetDistance(CitizenFX.Core.Game.Player.Character.Position, location);
                if (Distance <= 0.5f)
                {
                    Screen.DisplayHelpTextThisFrame("Press ~INPUT_PICKUP~ to enter theater");
                    if (API.IsControlJustPressed(0, 38))
                    {
                        CitizenFX.Core.Game.Player.Character.Position = new Vector3(-1427.299f, -245.1012f, 16.8039f);
                    }
                }
            }

            float ExitDistance = World.GetDistance(CitizenFX.Core.Game.Player.Character.Position, ExitLocation);
            if (ExitDistance <= 0.5f)
            {
                Screen.DisplayHelpTextThisFrame("Press ~INPUT_PICKUP~ to exit theater");
                if (API.IsControlJustPressed(0, 38))
                {
                    CitizenFX.Core.Game.Player.Character.Position = new Vector3(-1423.3f, -214.23f, 46.5f);
                }
            }
        }
