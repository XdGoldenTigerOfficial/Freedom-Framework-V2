...
namespace client.Apartments
{
    public class ApartmentMain : BaseScript
    {
        ...
        {
            float WestMirrorDrive = World.GetDistance(CitizenFX.Core.Game.Player.Character.Position, new Vector3(861.64f, -582.63f, 58.16f));
            if (WestMirrorDrive <= 0.5f)
            {
                Screen.DisplayHelpTextThisFrame("Press ~INPUT_PICKUP~ to enter house");
                if (CitizenFX.Core.Game.IsControlJustPressed(0, Control.Pickup))
                {
                    TriggerServerEvent("Freedom:EnterHouse");
                    CitizenFX.Core.Game.Player.Character.Position = LowEndApartment;
                }
            }

            float WestMirrorDriveExit = World.GetDistance(CitizenFX.Core.Game.Player.Character.Position, new Vector3(265.94f, -1003.21f, -99.01f));
            if (WestMirrorDriveExit <= 0.5f)
            {
                ...
                if (CitizenFX.Core.Game.IsControlJustPressed(0, Control.Pickup))
                {
                    ...
                    CitizenFX.Core.Game.Player.Character.Position = new Vector3(861.64f, -582.63f, 58.16f);
                }
            }

            float WildOatsDriveEnter = World.GetDistance(CitizenFX.Core.Game.Player.Character.Position, new Vector3(-175.83f, 502.43f, 137.42f));
            if (WildOatsDriveEnter <= 0.5f)
            {
                ...
                {
                    TriggerServerEvent("Freedom:EnterHouse");
                    CitizenFX.Core.Game.Player.Character.Position = WildOatsDrive3655;
                }
            }

            float WildOatsDriveExit = World.GetDistance(CitizenFX.Core.Game.Player.Character.Position, new Vector3(-174.14f, 497.18f, 137.67f));
            if (WildOatsDriveExit <= 0.5f)
            {
                ...
                {
                    TriggerServerEvent("Freedom:ExitHouse");
                    CitizenFX.Core.Game.Player.Character.Position = new Vector3(-175.83f, 502.43f, 137.42f);
                }
            }

            float MirrorParkBlvd = World.GetDistance(CitizenFX.Core.Game.Player.Character.Position, new Vector3(1220.29f, -687.94f, 60.79f));
            if (MirrorParkBlvd <= 0.5f)
            {
                ...
                {
                    TriggerServerEvent("Freedom:EnterHouse");
                    CitizenFX.Core.Game.Player.Character.Position = MediumEndApartment;
                }
            }

            float MirrorParkBlvdExit = World.GetDistance(CitizenFX.Core.Game.Player.Character.Position, new Vector3(346.76f, -1011.29f, -99.2f));
            if (MirrorParkBlvdExit <= 0.5f)
            {
                ...
                {
                    TriggerServerEvent("Freedom:ExitHouse");
                    CitizenFX.Core.Game.Player.Character.Position = new Vector3(1220.29f, -687.94f, 60.79f);
                }
            }
        }
