...
namespace client.Dev
{
    public class DevMenu : BaseScript
    {
        ...
        private void MenuItems(UIMenu menu)
        {
            ...
            devMenu.OnListChange += (sender, item, index) =>
            {
                if (item == selectspawn)
                {
                    if (selectspawn.CurrentItem() == "2 Car Garage")
                    {
                        CitizenFX.Core.Game.Player.Character.Position = Apartments.ApartmentMain.TwoCarGarage;
                    }

                    if (selectspawn.CurrentItem() == "6 Car Garage")
                    {
                        CitizenFX.Core.Game.Player.Character.Position = Apartments.ApartmentMain.SixCarGarage;
                    }

                    if (selectspawn.CurrentItem() == "10 Car Garage")
                    {
                        CitizenFX.Core.Game.Player.Character.Position = Apartments.ApartmentMain.TenCarGarage;
                    }

                    if (selectspawn.CurrentItem() == "Low End Apartment")
                    {
                        CitizenFX.Core.Game.Player.Character.Position = Apartments.ApartmentMain.LowEndApartment;
                    }

                    if (selectspawn.CurrentItem() == "Medium End Apartment")
                    {
                        CitizenFX.Core.Game.Player.Character.Position = Apartments.ApartmentMain.MediumEndApartment;
                    }
                }
            };
        }
